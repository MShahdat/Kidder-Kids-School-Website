  ...
  return (
    <div className=" w-full">
      <Slider {...settings} className="h-screen overflow-hidden">
        {hero.map((ba, idx) => (
          <div key={idx} className="relative h-screen"> {/* FIXED */}
            ...
            <div className="relative max-w-7xl mx-auto text-white h-full flex items-center px-4 sm:px-8">
              <div className="absolute left-4 md:left-8 top-1/2 -translate-y-3/4"> {/* now works properly */}
                <div className="flex flex-col gap-8 w-[80%] md:w-[70%] ">
                  ...
                  <div className="flex items-center gap-4">
                    <Link to={'/about'} className="px-4 sm:px-8 py-2.5 bg-orange-600 font-medium text-[14px] rounded-full">
                      Learn More
                    </Link>
                    <Link to={'/classes'} className="px-4 sm:px-8 py-2.5 bg-[#140d3d] font-medium text-[14px] rounded-full">
                      Our Classes
                    </Link>
                  </div>
                </div>
